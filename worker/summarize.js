...
export async function createStructuredSummary(chunkSummaries, filename) {
  ...
  const completion = await openai.chat.completions.create({
    model: "gpt-4o-mini",
    messages: [
      { role: "system", content: "Create a structured JSON summary." },
      {
        role: "user",
        content: `Using the chunk summaries below, create a JSON object with:
- "overview": 3–5 sentence plain-text overview
- "keyPoints": 5–8 bullet points, plain text only
